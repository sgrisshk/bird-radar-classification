...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build hard-negative bank from OOF predictions.")
    ...
    p.add_argument("--topq", type=float, default=0.98, help="Quantile on negative p_c for hard negative cut.")
    p.add_argument("--require-top1-gulls", action="store_true")
...
def main() -> None:
    ...
    top1 = np.argmax(oof, axis=1).astype(np.int64)
    gulls_idx = int(CLASS_TO_INDEX["Gulls"])
    ...
    for cls in focus_classes:
        ...
        neg_mask = y_idx != c
        p_neg = oof[neg_mask, c]
        if len(p_neg) <= 0:
            bank[cls] = []
            stats[cls] = {"n_neg": 0, "threshold": float("nan"), "selected": 0}
            continue
        thr = float(np.quantile(p_neg, topq))
        sel = neg_mask & (oof[:, c] >= thr)
        if bool(args.require_top1_gulls):
            sel = sel & (top1 == gulls_idx)
        ...
        stats[cls] = {
            ...
            "selected_ratio": float(len(ids) / max(1, int(neg_mask.sum()))),
        }
    ...
    report = {
        ...
        "topq": topq,
        "require_top1_gulls": bool(args.require_top1_gulls),
        "stats": stats,
        "bank": bank,
    }
